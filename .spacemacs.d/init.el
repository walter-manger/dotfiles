...
;; Think more about this minimal configuration
...
before packages are loaded."
  ...
  (setq sql-connection-alist
        '((admin_dev (sql-product 'postgres)
                   (sql-port 3306)
                   (sql-postgres-options)
                   (sql-server "172.0.0.1")
                   (sql-user "admin_dev")
                   (sql-password "UwYTsaR5nFxm8NEBmFmL98Ux")
                   (sql-database "admin_app"))
	(local_postgres (sql-product 'postgres)
                    (sql-port 5432)
                    (sql-postgres-options)
                    (sql-server "172.0.0.1")
                    (sql-user "postgres")
  ...
  (defun wm/work ()
    (interactive)
    (find-file "~/Dropbox/Org/organizer/.agenda-files/work.org")
    )

  (defun wm/home ()
    (interactive)
    (find-file "~/Dropbox/Org/organizer/.agenda-files/home.org")
    )

  (defun wm/play-chill ()
    "Start up some nice Jazz"
    (interactive)
    (emms-play-url "http://chillstep.info:1984/listen.mp3"))

  (defun wm/play-drone ()
    "Start up some nice Drone"
    (interactive)
    (emms-play-file "~/Downloads/dronezone256.pls"))

  (defun wm/play-ambient ()
    "Start up some nice Ambient"
    (interactive)
    (emms-play-file "~/Downloads/pill-hi-mp3.pls"))
...
(custom-set-variables
 ...
 '(custom-safe-themes
   (quote
    ("99ea831ca79a916f1bd789de366b639d09811501e8c092c85b2cb7d697777f93" "e074be1c799b509f52870ee596a5977b519f6d269455b84ed998666cf6fc802a" "845103fcb9b091b0958171653a4413ccfad35552bc39697d448941bcbe5a660d" "e1ecb0536abec692b5a5e845067d75273fe36f24d01210bf0aa5842f2a7e029f" default)))
